<header class="min-h-28 p4 bg-gray-500  p-4">
        ...
        <div class="flex flex-row">
            <div class="flex items-center gap-10 md:gap-20">
                ...
                @if(Request()->path() === 'festival')
                    <a href="{{route('festival.index')}}" class=" border-b-4 text-xl md:text-2xl text-white">Festivals</a>
                @else
                    <a href="{{route('festival.index')}}" class="text-xl md:text-2xl text-white">Festivals</a>
                @endif
                ...
                @auth
                    @if(Request()->user()->role === 'Admin')
                        @if(Request()->path() === "admin-dashboard")
                            <a href="{{route('admin-dashboard')}}" class="border-b-4 text-xl md:text-2xl text-white">Dashboard</a>
                        @else
                            <a href="{{route('admin-dashboard')}}" class=" text-xl md:text-2xl text-white">Dashboard</a>
                        @endif
                    @else
